...
class Solution1 {//36 test passed
public:
    int maxArea(vector<int>& height) {
        if(height.size()<2)
            return 0;
        map<int,int>point;
        int n=height.size(),maxc=0,h=0,v=0;
        int result[n][n]={0};
        ...
        for(int i=0;i<n-1;i++)
            for(int j=i+1;j<n;j++)
        {
            result[i][j]=(j-i)*min(height[i],height[j]);
